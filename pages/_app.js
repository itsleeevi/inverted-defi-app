...
import spLPABI from "../artifacts/spLP.json";
...
function MyApp({ Component, pageProps }) {
  ...
  const [tokenContractHttp, setTokenContractHttp] = useState(undefined);
  ...
  const [yourBalanceLp, setYourBalanceLp] = useState(undefined);
  ...
  const url = "https://timer15min.herokuapp.com/deadline";

  useEffect(() => {
    const intervalId = setInterval(() => {
      axios.get(url).then((response) => {
        //console.log(response.data);
        setDeadline(Number(response.data));
      });
    }, 1000);
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      setTime(Math.floor((deadline - Date.now()) / 60000));
    }, 1000);
  ...
  const Treasury = async () => {
    const result = await tokenContractHttp.methods
  ...
  const approveDepositToken = async () => {
    ...
    const tx = depositTokenContract.methods.approve(
      CONFIG.POOL_ADDRESS_NOLOCK,
      Converter.toHex(MAX_AMOUNT)
    );
  ...
  useEffect(() => {
    ...
    if (stakingContractHttp && tokenContractHttp) init();
  }, [stakingContractHttp, tokenContractHttp, refresh, staked]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      init();
  }, [accounts, tokenContract, stakingContract, depositTokenContract, staked]);
    ...
    if (
      ...
      const isApproved = async () => {
        ...
        await depositTokenContract.methods
          ...
          .call()
          .then((result) => {
            //console.log("allowance 7 days", result);
            if (Number(result) === 0) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const tokenContractHttp = new web3Http.eth.Contract(
        tokenABI,
        CONFIG.TOKEN_ADDRESS
      );
      ...
      setTokenContractHttp(tokenContractHttp);
    };
